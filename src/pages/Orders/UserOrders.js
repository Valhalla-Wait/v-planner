...
export default function UserOrders() {
  ...
  const ordersList = useSelector(state => {
    if(state.quotes.quotesList) {
      return state.quotes.quotesList.filter((quote) => quote.status === 'ACCEPTED' || quote.status === 'DECLINED')
    }else{ 
      return null
    }
  })
                  ...
                  <div className="table__cell cell-company">
                    <div className="cell-company__content">
                      <div className="cell-company__img">
                        <img src={order.vendor.vendorModel.photos[0].url} alt={order.vendor.firstName} />
                      </div>
                      <div className="cell-company__info">
                        <div className="cell-company__name">{order.vendor.firstName} {order.vendor.surname}</div>
                        <div className="cell-company__type">{ order.vendor.vendorModel.companyName }</div>
                      </div>
                    </div>
                  </div>
